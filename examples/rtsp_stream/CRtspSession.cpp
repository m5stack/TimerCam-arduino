...
void CRtspSession::Handle_RtspOPTION() {
    static char Response[1024];  // Note: we assume single threaded, this large
...
void CRtspSession::Handle_RtspDESCRIBE() {
    static char Response[4096];  // Note: we assume single threaded, this large
                                 // buf we keep off of the tiny stack
    ...
    switch (m_StreamID) {
        case 0:
            strcpy(StreamName, "mjpeg/1");
            break;
        case 1:
            strcpy(StreamName, "mjpeg/2");
            break;
    };
    snprintf(URLBuf, sizeof(URLBuf), "rtsp://%s/%s", m_URLHostPort, StreamName);
    snprintf(Response, sizeof(Response),
             "RTSP/1.0 200 OK\r\nCSeq: %s\r\n"
             "%s\r\n"
             "Content-Base: %s/\r\n"
             ...
             "Content-Length: %d\r\n\r\n"
             "%s",
             m_CSeq, DateHeader(), URLBuf, (int)strlen(SDPBuf), SDPBuf);
...
void CRtspSession::InitTransport(u_short aRtpPort, u_short aRtcpPort) {
    ...
    if (!m_TcpTransport) {  // allocate port pairs for RTP/RTCP ports in UDP
                            // transport mode
...
void CRtspSession::Handle_RtspPLAY() {
    ...
    snprintf(Response, sizeof(Response),
             ...
             m_CSeq, DateHeader(), m_RtspSessionID);
