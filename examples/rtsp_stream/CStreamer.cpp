...
void CStreamer::addSession(WiFiClient &aClient) {
    ...
    new CRtspSession(aClient, this);  // our threads RTSP session and state
...
#define MAX_FRAGMENT_SIZE 1100  // FIXME, pick more carefully
    ...
    // in case of TCP based transport
    RtpBuf[0] = '$';  // magic number
    RtpBuf[1] = 0;    // number of multiplexed subchannel on RTPS connection -
                      // here the RTP channel
    ...
    RtpBuf[20] = 0x00;          // type (fixme might be wrong for camera data)
    ...
    if (includeQuantTbl) {  // we need a quant header - but only in first packet
                            // of the frame
    ...
    // printf("Sending timestamp %d, seq %d, fragoff %d, fraglen %d, jpegLen
    // %d\n", m_Timestamp, m_SequenceNumber, fragmentOffset, fragmentLen,
    // jpegLen);
    ...
    while (element != &m_Clients) {
        ...
        if (session->m_streaming && !session->m_stopped) {
            if (session->isTcpTransport())  // RTP over RTSP - we send the
                                            // buffer + 4 byte additional header
                socketsend(session->getClient(), RtpBuf, RtpPacketSize + 4);
            else  // UDP - we send just the buffer by skipping the 4 byte RTP
                  // over RTSP header
            {
...
bool CStreamer::InitUdpTransport(void) {
    ...
    for (u_short P = 6970; P < 0xFFFE; P += 2) {
        ...
        if (m_RtpSocket) {  // Rtp socket was bound successfully. Lets try to
                            // bind the consecutive Rtsp socket
        ...
        if (typecode == marker) {
            ...
            *len -= skipped;
            ...
            switch (typecode) {
                ...
                case 0xda:  // sos
                {
                    // standard format section with 2 bytes for len.  skip that
                    // many bytes
...
// followed by something not zero, is a new section.  Look for that marker and
// return the ptr pointing there
...
bool decodeJPEGfile(BufPtr *start, uint32_t *len, BufPtr *qtable0, BufPtr *qtable1) {
    ...
    if (!findJPEGheader(&bytes, len,
                        0xd8))  // better at least look like a jpeg file
        return false;           // FAILED!
    ...
    *qtable1          = NULL;
    ...
    if (!findJPEGheader(start, len, 0xda)) return false;  // FAILED!
    ...
    *len -= soslen;
    ...
    if (!findJPEGheader(&endmarkerptr, &endlen, 0xd9)) return false;  // FAILED!
